...
clustersizes <- function(clust,max.clusters){
  ...
  mx <- matrix(data=rep(0,max.clusters*max.clusters),
               nrow = max.clusters,ncol=max.clusters)
...
plot(sz_test(sz_single),type = 'o',ylim = c(0,1),xlim=c(1,25),
     col='orange3',xlab = 'Nr. of cluster',ylab = 'Max. clustersize')
...
legend(20.5,0.8,c('single','complete','average','centroid','mcquitty',
                  'ward.D','ward.D2'),pch=c(1,1,1,1,1,1,1),
       lty = c(1,1,1,1,1,1,1),
       col = c('orange3','black','aquamarine','darkorchid','green',
               'red','blue'))
...
print(paste('Percentage of Trump voters in US Data:',
            as.character(100*round(sum(dt$trump)/
                                     sum(dt$X.dem..country_code=='US'),
                                   digits = 4))))
...
trump_voters <- function(clust,max.clusters){
  #computes the proportion of trump voters compared to total US respondents
  #in each cluster and returns results in a matrix
  mx <- matrix(data=rep(0,max.clusters*max.clusters),
               nrow = max.clusters,ncol=max.clusters)
  for (i in 1:max.clusters){
    helper <- cutree(clust,k=i)
    for (j in 1:i){
      if ((sum((dt$X.dem..country_code=='US')*helper==j)==0)){
        ...
        mx[i,j]=sum(dt$trump*helper==j)/
          sum((dt$X.dem..country_code=='US')*helper==j)
      }
...
trump_metric <- function(trump_voters,clustersizes){
  #Computes a Metric for relevance of clusters
  #The score becomes higher if the clusters are able to seperate trump
  #and non-trump voters in bigger clusters
...
plot(metric_complete,type='o',
ylim=c(0,max(metric_complete,metric_ward.D,metric_ward.D2)+0.001),
ylab = 'Metric',xlab = 'Nr. of cluster',main = 'Comparison of metric for
     different methods and amount of clusters')
points(metric_ward.D,col='red',type = 'o')
points(metric_ward.D2,col='blue',type = 'o')

legend(15,0.04,c('complete','ward.D','ward.D2'),
       pch=c(1,1,1),lty=c(1,1,1),col = c('black','red','blue'))
...
# trump_majority <- function(trump_voters){
#   size <- dim(trump_voters)[1]
#   vec <- rep(0,size)
#   for(i in 1:size){
#     vec[i] <- max(trump_voters[i,1:i])
#   }
#   return(vec)
# }
# 
# majority_complete <- trump_majority(trump_complete)
# majority_ward.D <- trump_majority(trump_ward.D)
# majority_ward.D2 <- trump_majority(trump_ward.D2)
# 
# plot(majority_complete,ylab = 'Maximal proportion of Trump voters in a cluster',
#      xlab = 'Nr. of cluster',type = 'o',ylim = c(0.3,1),xlim = c(1,21))
# points(majority_ward.D,col='red',type = 'o')
# points(majority_ward.D2,col='blue',type = 'o')
# lines(rep(0.5,20),lty=2)
# legend(1,0.9,c('complete','ward.D','ward.D2','Majority'),
...
large_clust_majority <- function(trump_voters,clustersizes){
  #Finds clusters where the majority of US Voters voted for Trump
  #Returns the largest of these Clusters as Output Message
  size <- dim(trump_voters)[1]
  vec <- rep(0,4)
  for(i in 1:size){
    for(j in 1:i){
  ...
  print(paste('Found largest Cluster with Trump Majority with',
              as.character(100*round(vec[1],3)),
              'percentage of total respondents in Cluster',
              as.character(vec[3]),
              'with',as.character(vec[2]),'Clusters. Majority is:',
              as.character(round(vec[4],3))))
}
...
trump_barplot <- function(clust,num.clust){
  #Creates a barplot with all US respondents in a cluster seperated
  #by Trump voters and Non-Trump voters
  counts <- table(dt$trump[dt$X.dem..country_code=='US'],
                  cutree(clust,k=num.clust)[dt$X.dem..country_code=='US'])
  
  nm <-deparse(substitute(clust))
  
  barplot(counts,col=c("darkblue","red"),beside = TRUE,
          xlab = 'Nr. of Cluster',ylab='Amount of US Voters in Cluster',
          main = paste('Method:',nm,', Nr. of Cluster:',
                       as.character(num.clust)))
...
#Candidates from trump_metric: ward.D k=2/5, ward.D2 k=2/4
#Candidates from large_clust_majority: Complete k=18,Ward.D k=9,Ward.D2 k=7
...
out <- data.frame(cbind(dt$ID,ward.D_metric_1,ward.D_metric_2,
                        ward.D2_metric_1,ward.D2_metric_2,
                        ward.D_majority,ward.D2_majority,complete_majority))
names(out)[1] <- 'ID'
