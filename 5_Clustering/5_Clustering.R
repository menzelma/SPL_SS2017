...
plot(sz_test(sz_single),type = 'o',ylim = c(0,1),xlim=c(1,25),
     col='orange3',xlab = 'Nr. of cluster',ylab = 'Max. clustersize',lty=3,lwd=2)
...
legend(20.5,0.8,c('single','complete','average','centroid','mcquitty',
       ...
       lty = c(3,1,5,4,1,3,5),
       col = c('orange3','black','aquamarine','darkorchid','green3',
               'red','blue'))
...
legend(15,0.04,c('complete','ward.D','ward.D2'),
       pch=c(1,1,1),lty=c(1,3,5),col = c('black','red','blue'))

#Candidates ward.D 2/3/5, ward.D2 2/4 (local maxima)
#But the cluster=2 candidates are local maxima because they are 
#by definition better than the first split
...
#Candidates from trump_metric: ward.D k=5, ward.D2 k=4
...
out <- data.frame(cbind(dt$ID,ward.D_metric,ward.D2_metric,
