...
dt_common = rbind(dt_usa[, c(1:13, 15:41, 43:68)], dt_eur[, c(1:13, 15:38, 40:42, 44, 45, 67:90)])
...
# Assign values to the countries (5=North america, 1=Western,Northern, Southern Europe, 2=Eastern Europe) Following origin code in Survey
# Codebook
dt_origin$country_origin = (dt_common$X.dem..country_code == "US") * 5 + (dt_common$X.dem..country_code == "AT") * 1 + (dt_common$X.dem..country_code == 
    "BE") * 1 + (dt_common$X.dem..country_code == "CY") * 1 + (dt_common$X.dem..country_code == "DE") * 1 + (dt_common$X.dem..country_code == 
    "DK") * 1 + (dt_common$X.dem..country_code == "ES") * 1 + (dt_common$X.dem..country_code == "FI") * 1 + (dt_common$X.dem..country_code == 
    "FR") * 1 + (dt_common$X.dem..country_code == "GB") * 1 + (dt_common$X.dem..country_code == "GR") * 1 + (dt_common$X.dem..country_code == 
    "IE") * 1 + (dt_common$X.dem..country_code == "IT") * 1 + (dt_common$X.dem..country_code == "LU") * 1 + (dt_common$X.dem..country_code == 
    "MT") * 1 + (dt_common$X.dem..country_code == "NL") * 1 + (dt_common$X.dem..country_code == "PT") * 1 + (dt_common$X.dem..country_code == 
    "SE") * 1 + (dt_common$X.dem..country_code == "BG") * 2 + (dt_common$X.dem..country_code == "CZ") * 2 + (dt_common$X.dem..country_code == 
    "EE") * 2 + (dt_common$X.dem..country_code == "HR") * 2 + (dt_common$X.dem..country_code == "HU") * 2 + (dt_common$X.dem..country_code == 
    "LT") * 2 + (dt_common$X.dem..country_code == "LV") * 2 + (dt_common$X.dem..country_code == "PL") * 2 + (dt_common$X.dem..country_code == 
    "RO") * 2 + (dt_common$X.dem..country_code == "SI") * 2 + (dt_common$X.dem..country_code == "SK") * 2

# Create a helper variable that takes the first value from the origin variable
origin_helper = as.numeric(sub("^\\D*(\\d+).*$", "\\1", as.character(dt_common$X.dem..origin)))
...
dt_origin$Origin1 = (dt_common$X.dem..immigration == 4) * dt_origin$country_origin + (dt_common$X.dem..immigration != 4) * as.numeric(origin_helper)

# Set all zeroes to NA (there was no value neither in in the country_origin nor in the origin variable)
...
# create a helper variable and insert the value from social networks regualrly used as string and insert spaces at the beginning and the
# end so that we have always the patter space+value+space in the variable
dt_social$social_media_helper = paste(" ", as.character(dt_common$X.aud..social_networks_regularly_used), " ", sep = "")

# For all possible answers loop go trough the string created above and look if pattern space+number+space exists.if true write 1 to the
# dummy for this value
for (i in 1:16) {
...
# Create a new data frame to encode each respondents first and second most done activites in social networks
dt_social_activity = data.frame(dt_common$ID)

# Find the position in the answer where the respondent assigned a 1(most often) and 2 (second most)
frst = gregexpr(pattern = "1", as.character(dt_common$X.aud..social_media_activity_rank))
snd = gregexpr(pattern = "2", as.character(dt_common$X.aud..social_media_activity_rank))

# As the order is fixed, the positons in the answer are encoded due to the activity
...
dt_member$member_sports = grepl("1", as.character(dt_common$X.aud..member_organization), fixed = TRUE) * 1
dt_member$member_party = grepl("2", as.character(dt_common$X.aud..member_organization), fixed = TRUE) * 1
dt_member$member_church = grepl("3", as.character(dt_common$X.aud..member_organization), fixed = TRUE) * 1
dt_member$member_other = grepl("4", as.character(dt_common$X.aud..member_organization), fixed = TRUE) * 1
dt_member$member_none = grepl("5", as.character(dt_common$X.aud..member_organization), fixed = TRUE) * 1
...
# Create a helper variable and insert spaces at the beginning and end of the answer so that each answer has the form space+value+space
spaces = paste(" ", as.character(dt_common$X.question..important_issues_when_voting), " ", sep = "")

dt_importance$importance_helper = spaces
rm(spaces)

# For each possible answer(1 to 16) encode them to dummy variables if they occur in respondents answer
for (i in 1:16) {
    nr = paste(" ", as.character(i), " ", sep = "")
    dt_importance[, i + 2] = grepl(nr, dt_importance$importance_helper, fixed = TRUE) * 1
    names(dt_importance)[i + 2] = paste("importance_dummy_", as.character(i), sep = "")
}
...
# Find the position in the answer where the respondent assigned a 1(most preferred) and 2 (second most)
frst = gregexpr(pattern = "1", as.character(dt_common$X.question..preferred_type_of_political_leader))
snd = gregexpr(pattern = "2", as.character(dt_common$X.question..preferred_type_of_political_leader))

# As the order is fixed, the positons in the answer are encoded due to the leader type
...
# In order to do a clustering ordered type variables instead of factors are used to create more expressivness for the similiarity measure.
# As sometimes a option for non-answering the question was given, this option has to be set to NA as this option cannot be part of an
# ordered variable
dt_common$X.dem..income_net_monthly[dt_common$X.dem..income_net_monthly == 13] = NA

dt_common$X.dem..education_level[dt_common$X.dem..education_level == 5] = NA

dt_common$X.dem..disposable_income[dt_common$X.dem..disposable_income == 6] = NA

dt_common$X.dem..household_finances_past12months[dt_common$X.dem..household_finances_past12months == 6] = NA

dt_common$X.dem..financial_security[dt_common$X.dem..financial_security == 5] = NA

dt_common$X.dem..change_household_finances_next12months[dt_common$X.dem..change_household_finances_next12months == 6] = NA

dt_common$X.dem..job_security[dt_common$X.dem..job_security == 5] = NA

dt_common$X.dem..status_national_economy[dt_common$X.dem..status_national_economy == 6] = NA

dt_common$X.dem..change_economy_country_past12months[dt_common$X.dem..change_economy_country_past12months == 6] = NA

dt_common$X.dem..change_household_finances_next12months[dt_common$X.dem..change_household_finances_next12months == 6] = NA

dt_common$X.question..likelihood_to_demonstrate[dt_common$X.question..likelihood_to_demonstrate == 5] = NA

dt_common$X.question..frequency_of_voting[dt_common$X.question..frequency_of_voting == 6] = NA

dt_common$X.question..vote_next_national_election[dt_common$X.question..vote_next_national_election == 5] = NA

dt_common$X.question..currentplace_change_past5years[dt_common$X.question..currentplace_change_past5years == 6] = NA

dt_common$X.question..hometown_change_past5years[dt_common$X.question..hometown_change_past5years == 6] = NA

dt_common$X.question..family_friends_highereducation[dt_common$X.question..family_friends_highereducation == 7] = NA

# Create a data set for the clustering approach
data_cluster = cbind(dt_common, dt_origin[, 2], dt_social[, c(2:17)], dt_social_activity[, c(2:11)], dt_member[, c(2:6)], dt_importance[, 
    c(3:18)], dt_leader[, c(2:13)])
...
data_cluster[, c(5, 7, 8, 10, 12:15, 18, 40:43, 59:119)] = lapply(data_cluster[, c(5, 7, 8, 10, 12:15, 18, 40:43, 59:119)], as.factor)
data_cluster[, c(6, 11, 16, 17, 19:39, 44:58)] = lapply(data_cluster[, c(6, 11, 16, 17, 19:39, 44:58)], as.ordered)

# Set the variable levels for the complete data set
data_preprocessed[, c(5:8, 10:119)] = lapply(data_preprocessed[, c(5:8, 10:110)], as.factor)
...
dt_usa = cbind(dt_usa, dt_social[c(1:1052), c(2:17)], dt_social_activity[c(1:1052), c(2:11)], dt_member[c(1:1052), c(2:6)], dt_importance[c(1:1052), 
    c(3:18)], dt_leader[c(1:1052), c(2:13)])
...
# Extract the most favorite and least favorite party for each German respondent
...
# Extract the most favorite and least favorite party for each respondent from France
...
# Extract the most favorite and least favorite party for each respondent from Great Britain
...
dt_eur = cbind(dt_eur, dt_social[c(1053:12335), c(2:17)], dt_social_activity[c(1053:12335), c(2:11)], dt_member[c(1053:12335), c(2:6)], dt_importance[c(1053:12335), 
    c(3:18)], dt_leader[c(1053:12335), c(2:13)])

# Delete unencoded variables in the EUR data set
dt_eur[, c(14, 30, 31, 34, 67, 68, 69)] = NULL

# Create data sets for Germany, France and Great Britain each with country specific questions
