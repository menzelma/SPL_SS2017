...
proportions <- function(col_nr_clust,trump_prop,sz,
                        dt_clust.=dt_clust,DE_clust.=DE_clust,
                        FR_clust.=FR_clust,GB_clust.=GB_clust){
  ...
  for(i in 1:nr_clust){
    ...
    if(sum(DE_clust.[,col_nr_clust]==i)>0){
      df[i,2] <- (sum(party_DE$AFD_first[DE_clust.[,col_nr_clust]==i]==1)/
                    sum(DE_clust.[,col_nr_clust]==i))/AFD_prop
    } else {
      df[i,2] <- 0
    }
    if(sum(FR_clust.[,col_nr_clust]==i)>0){
      df[i,3] <- (sum(party_FR$FN_first[FR_clust.[,col_nr_clust]==i]==1)/
                    sum(FR_clust.[,col_nr_clust]==i))/FN_prop
    }else{
      df[i,3] <- 0
    }
    if(sum(GB_clust.[,col_nr_clust]==i)>0){
      df[i,4] <- (sum(party_GB$UKIP_first[GB_clust.[,col_nr_clust]==i]==1)/
                    sum(GB_clust.[,col_nr_clust]==i))/UKIP_prop
    }else{
...
plot_result <- function(res,title){
  res <- res[ order(res[,1]), ]
  plot(res[,1],type = 'o',col='black',
       ylim = c(0,max(res[,c(1:4)])+1),
       ylab = 'Noramlized proportions of party voters',main = title,
  ...
  legend(1,max(res[,c(1:4)])+1,colnames(res[,c(1:4)]),
         pch=c(1,1,1,1),lty=c(1,3,4,5),col=c('black','red','blue','green3'))
}
...
correl <- function(res1,res2,res3,res4,res5){
  ...
  rownames(df) <- c(deparse(substitute(res1)),
                    deparse(substitute(res2)),
                    deparse(substitute(res3)),
                    deparse(substitute(res4)),
                    deparse(substitute(res5)))
  df[1,] <- c(corr(res1[,c(1,2)],w=res1[,5]),
              corr(res1[,c(1,3)],w=res1[,5]),
              corr(res1[,c(1,4)],w=res1[,5]))
  df[2,] <- c(corr(res2[,c(1,2)],w=res2[,5]),
              corr(res2[,c(1,3)],w=res2[,5]),
              corr(res2[,c(1,4)],w=res2[,5]))
  df[3,] <- c(corr(res3[,c(1,2)],w=res3[,5]),
              corr(res3[,c(1,3)],w=res3[,5]),
              corr(res3[,c(1,4)],w=res3[,5]))
  df[4,] <- c(corr(res4[,c(1,2)],w=res4[,5]),
              corr(res4[,c(1,3)],w=res4[,5]),
              corr(res4[,c(1,4)],w=res4[,5]))
  df[5,] <- c(corr(res5[,c(1,2)],w=res5[,5]),
              corr(res5[,c(1,3)],w=res5[,5]),
              corr(res5[,c(1,4)],w=res5[,5]))
  print('Correaltions')
  return(df)
}
...
proportions_country <- function(col_nr_clust,trump_prop,sz,country_clust,country_dt,
                        dt_clust.=dt_clust){
  nr_clust <- max(dt_clust.[,col_nr_clust])
  ...
  for(i in 1:nr_clust){
    ...
    for(j in 1:dim(country_dt)[2]){
      if(sum(country_clust[,col_nr_clust]==i)>0){
        df[i,j+1] <- (sum(country_dt[country_clust[,col_nr_clust]==i,j]==1)/
                      sum(country_clust[,col_nr_clust]==i))
      } else {
        df[i,j+1] <- 0
      }
    }
...
correl_DE <- function(res1,res2,res3,res4,res5){
  ...
  rownames(df) <- c(deparse(substitute(res1)),
                    ...
                    deparse(substitute(res4)),
                    deparse(substitute(res5)))
  df[1,] <- c(corr(res1[,c(1,2)],w=res1[,8]),
              corr(res1[,c(1,3)],w=res1[,8]),
              corr(res1[,c(1,4)],w=res1[,8]),
              corr(res1[,c(1,5)],w=res1[,8]),
              corr(res1[,c(1,6)],w=res1[,8]),
              corr(res1[,c(1,7)],w=res1[,8]))
  df[2,] <- c(corr(res2[,c(1,2)],w=res2[,8]),
              corr(res2[,c(1,3)],w=res2[,8]),
              corr(res2[,c(1,4)],w=res2[,8]),
              corr(res2[,c(1,5)],w=res2[,8]),
              corr(res2[,c(1,6)],w=res2[,8]),
              corr(res2[,c(1,7)],w=res2[,8]))
  df[3,] <- c(corr(res3[,c(1,2)],w=res3[,8]),
              corr(res3[,c(1,3)],w=res3[,8]),
              corr(res3[,c(1,4)],w=res3[,8]),
              corr(res3[,c(1,5)],w=res3[,8]),
              corr(res3[,c(1,6)],w=res3[,8]),
              corr(res3[,c(1,7)],w=res3[,8]))
  df[4,] <- c(corr(res4[,c(1,2)],w=res4[,8]),
              corr(res4[,c(1,3)],w=res4[,8]),
              corr(res4[,c(1,4)],w=res4[,8]),
              corr(res4[,c(1,5)],w=res4[,8]),
              corr(res4[,c(1,6)],w=res4[,8]),
              corr(res4[,c(1,7)],w=res4[,8]))
  df[5,] <- c(corr(res5[,c(1,2)],w=res5[,8]),
              corr(res5[,c(1,3)],w=res5[,8]),
              corr(res5[,c(1,4)],w=res5[,8]),
              corr(res5[,c(1,5)],w=res5[,8]),
              corr(res5[,c(1,6)],w=res5[,8]),
              corr(res5[,c(1,7)],w=res5[,8]))
  print('Correlations')
  return(df)
}
