...
x2test <- function(party_dummy,data, col.nr){
  #Computes the chisq statistics for a categorical variable vs. a dummy
  #and checks the conditions that the expected frequencies are all bigger 
  #than 1 and that less than 20% of the expected frequencies are bigger than 5
  #Returns a vector with Column Name, Column number, p-Value from Chi-squared 
  #test, Indicator if all expected values are bigger than 1,
  ...
  if(sum(chisq.test(kt)$expected>=5)/
     (dim(chisq.test(kt)$expected)[1]*dim(chisq.test(kt)$expected)[2])<0.8)
    {
    expVal_less5 <- 1
  }
...
results <- data.frame(VarName=character(),
                      Column=numeric(),
                      p_value=numeric(),
                      Reject_H0=numeric(),
                      expVal_less1=numeric(),
                      expVal_less5=numeric(),
                      stringsAsFactors = FALSE)

#init counter
cnt <- 1

#iterate over all categorical variables in the data set and check with
#chi-squared if they are independent of the decision to vote for trump
for(i in 5:120){
